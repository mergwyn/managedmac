...
Puppet::Type.type(:remotemanagement).provide(:default) do
  ...
  DSLOCAL_USERS_DIR = '/private/var/db/dslocal/nodes/Default/users'.freeze
  ARD_PREFERENCES   = '/Library/Preferences/com.apple.RemoteManagement.plist'.freeze
  VNC_PASSWORD_FILE = '/Library/Preferences/com.apple.VNCSettings.txt'.freeze
  VNC_SEED          = '1734516E8BA8C5E2FF1C39567390ADCA'.freeze
  ...
  class << self
    ...
    def service_properties
      ...
      { name: 'apple_remote_desktop',
        ...
        users: all_ard_users }.delete_if { |_k, v,| v.nil? }
    end

    def launchd_file_exists?
      ['/private/etc',
       '/Library/Application Support/Apple/Remote Desktop'].each { |p| File.exist? File.join(p, 'RemoteManagement.launchd') }.any?
    end
    ...
    def read_vnc_password(path)
      result = nil
      if File.exist?(path)
        if file = File.read(path) # rubocop:disable Lint/AssignmentInCondition
          if file.length == 32
            seed = convert_to_hex VNC_SEED
            pass = convert_to_hex file
            result = seed.reduce('') do |memo, byte|
    ...
    def convert_to_hex(string)
      string.scan(%r{..}).map { |byte| byte.hex }
    end
  ...
  def initialize(value = {})
    super(value)
    @property_hash  = self.class.service_properties
    @property_flush = {}
  end
  ...
  def configure_access
    ...
    assigned_users = resource[:users].each_with_object({}) do |(k, v), memo|
      if validate_user(k)
        memo[k] = v
      else
    ...
    existing_users = self.class.all_ard_users
