...
Puppet::Type.type(:dsconfigad).provide(:default) do
  ...
  class << self
    ...
    def transform_config(config)
      config = DSCONFIGAD_KEY_MAP.each_with_object({}) do |(k, v), memo|
    ...
    def flatten_config(config)
      config.each_with_object({}) { |(_k, v), memo| memo.merge! v; }
    end
  ...
  def flag_setter(setter, value, accepts_no_flag = false)
    ...
    args = if (value.to_s.empty? || value == :absent) && accepts_no_flag
             ["-no#{flag}"]
           else
             ["-#{flag}", value]
           end
  ...
  def validate_bind_value(key, value)
    if value.nil? || value.empty? || value == :absent
      raise Puppet::Error,
  ...
  def normalize_bind_args(params_hash)
    params_hash.each_with_object([]) do |(k, v), memo|
  ...
  def build_configuration_options
    ...
    resource.to_hash.each do |k, v|
      send("#{k}=", v) if properties.member? k
    end
    @configuration_flags
  end
