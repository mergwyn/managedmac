Puppet::Type.newtype(:dsconfigad) do
  ...
  newparam(:fqdn) do
    desc 'The fully-qualified DNS name of the Domain to be used when adding
  ...
  newparam(:username) do
    desc 'Username of a Network account that has administrative privileges
  ...
  newparam(:password) do
    desc 'Password to use in conjunction with the specified username.'
    validate do |value|
      unless value.is_a? String
        raise ArgumentError, "Expected String, got #{value.class}"
      end
  ...
  newparam(:computer) do
    desc 'The "computerid" to add the specified Domain'
    validate do |value|
      unless value.is_a? String
        raise ArgumentError, "Expected String, got #{value.class}"
      end
  ...
  newparam(:ou) do
    desc 'The LDAP DN of the container to use for adding the computer.  If
  ...
  newparam(:force) do
    desc 'Force the process (i.e., join the existing account or remove the
      binding)'
  ...
  newparam(:leave) do
    desc 'Leaves the current domain (preserving the computer record in the
      directory).'
    newvalues(:enable, :disable)
    defaultto(:disable)
  end
  ...
  newproperty(:mobile) do
    desc 'This flag determines whether the plugin will enable mobile account
      support for offline logon.'
    newvalues(:enable, :disable)
  end

  newproperty(:mobileconfirm) do
    desc 'This flag determines whether the plugin will warn the user when a
      mobile account is going to be created.'
    newvalues(:enable, :disable)
  end

  newproperty(:localhome) do
    desc 'This flag determines whether the plugin forces all home directories
      to be local to the computer (i.e. /Users/username)'
    newvalues(:enable, :disable)
  end

  newproperty(:useuncpath) do
    desc 'This flag determines whether the plugin uses the UNC specified in
  ...
  newproperty(:protocol) do
    desc 'This flag determines how a home directory is mounted on the
  ...
  newproperty(:sharepoint) do
    desc 'Enable or disable mounting of the network home as a sharepoint.'
    newvalues(:enable, :disable)
  end

  newproperty(:shell) do
    desc 'Use the specified shell (e.g., "/bin/bash") if a shell attribute
  ...
  newproperty(:uid) do
    desc 'This specifies the attribute to be used for the UID of the user. By
      default, a UID is generated from the Active Directory GUID.'
    validate do |value|
      unless value.is_a?(String) || value == :absent
        raise ArgumentError, "Expected String, got #{value.class}"
      end
    end
    def insync?(is)
  ...
  newproperty(:gid) do
    desc 'This specifies the attribute to be used for the GID of the user. By
  ...
  newproperty(:ggid) do
    desc 'This specifies the attribute to be used for the GID of the group.
  ...
  newproperty(:authority) do
    desc 'This feature is not described in the man page. Enable or disable
  ...
  newproperty(:preferred) do
    desc 'Use the specified server for all Directory lookups and
  ...
  newproperty(:groups, array_matching: :all) do
    desc 'Use the listed groups to determine who has local administrative
  ...
  newproperty(:alldomains) do
    desc 'This flag determines whether the plugin allows authentication from
  ...
  newproperty(:packetsign) do
    desc 'By default packet signing is allowed but not required, but can be
  ...
  newproperty(:packetencrypt) do
    desc 'By default packet encryption is allowed but not required, but can
  ...
  newproperty(:passinterval) do
    desc 'Set how often the computer trust account password should be changed
       (default 14 days).'
    newvalues(%r{^\d+$})
    munge do |value|
      value.to_i
  ...
  newproperty(:restrictddns, array_matching: :all) do
    desc 'Restricts Dynamic DNS updates to specific interfaces (e.g., en0, en1,
